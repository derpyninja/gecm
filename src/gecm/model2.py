...
LANDUSE_CHANGE = 0.25 # how long does it take until the new landuse returns profit.
SUBSIDIES = 0.8
...
def price_per_pixel(
    ...
    cattle_price_new = income_farmland_cattle + (
                tot_cattle / (income_farmland_cattle / income_farmland_sheep) / tot_sheep_0) * (
    ...
    c_forest_price_new = income_forest_commercial + tot_c_forest / (income_forest_commercial / income_forest_native) / (
                tot_n_forest + tot_c_forest) * (income_forest_native - income_forest_commercial)
...
def money_farmer(
    ...
    if current_round == 0:
        ...
        earning = (m_area + m_change + m_tourism + m_teamwork + m_brexit) * gdp_pc_scotland / (
...
def money_forester(
    ...
    if current_round == 0:
        ...
        earning = (m_area + m_change + m_tourism_factor + m_teamwork + m_brexit) * gdp_pc_scotland / (
                    100 * (area_n_forest[current_round] + area_c_forest[current_round]))
